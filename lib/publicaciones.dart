...
class _PublicarPageState extends State<PublicarPage> {
  ...
  Future<void> _cargarMaterias(String idFacultad) async {
    final data = await _supabase.from('materias').select().eq('id_facultad', idFacultad).order('nombre_materias');
    setState(() {
      _materias = data;
        ...
        child: Column(
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: _facultades.map((f) => DropdownMenuItem(
              ...
              children: [
                Expanded(
                  child: DropdownButtonFormField<String>(
                    ...
                    items: _materias.map((m) => DropdownMenuItem(
                      value: m['id_materias'].toString(),
                      child: Text(m['nombre_materias']),
                    )).toList(),
                    onChanged: _facultadSeleccionada == null ? null : (val) => setState(() => _materiaSeleccionada = val),
                  ),
                ),
              ],
            ),
              ...
              children: [
                Expanded(
                  ...
                  child: DropdownButtonFormField<String>(
                    ...
                    items: ['Parcial', 'Tarea', 'Quiz', 'Proyecto'].map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
            ...
            TextField(
              ...
              decoration: const InputDecoration(hintText: "Título descriptivo", border: UnderlineInputBorder()),
            ),
            const SizedBox(height: 20),
            ...
            TextField(
              ...
              maxLines: 3,
              decoration: const InputDecoration(hintText: "Añade tu problema específico", border: OutlineInputBorder()),
            ),
            const SizedBox(height: 20), 
                ...
                ? (_esPdf 
                    ? Column(
                        ...
                        children: [
                          ...
                          Text(_nombreArchivo ?? "", textAlign: TextAlign.center),
                        ],
                      )
                    : ClipRRect( 
                        borderRadius: BorderRadius.circular(10), 
                        child: Image.file(_archivoSeleccionado!, fit: BoxFit.cover),
                      ))
                : const Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.cloud_upload_outlined, size: 50, color: Colors.grey),
                      Text("Subir Foto o PDF", style: TextStyle(color: Colors.grey)),
                    ],
                  ),
              ),
            ),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor:  Color(0xFF007BFF), foregroundColor: Colors.white),
                child: const Text("SUBIR PUBLICACIÓN", style: TextStyle(fontSize: 16)),
              ),
            ),
          ],
        ),
